<div class="modal-dialog modal-lg" role="document">
  <div class="modal-content">
        ...
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('total_card_slips', __( 'cash_register.total_card_slips' ) . ':*') !!} @show_tooltip(__('tooltip.total_card_slips'))
              {!! Form::number('total_card_slips', $register_details->total_card_slips, ['class' => 'form-control', 'required', 'placeholder' => __( 'cash_register.total_card_slips' ), 'min' => 0, 'step' => 'any' ]); !!}
          </div>
        </div> 
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('total_cheques', __( 'cash_register.total_cheques' ) . ':*') !!} @show_tooltip(__('tooltip.total_cheques'))
              {!! Form::number('total_cheques', $register_details->total_cheques, ['class' => 'form-control', 'required', 'placeholder' => __( 'cash_register.total_cheques' ), 'min' => 0, 'step' => 'any' ]); !!}
          </div>
        </div> 

        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('total_pix', 'Pago PIX:*') !!}
              {!! Form::number('total_pix', $register_details->total_pix ?? 0, ['class' => 'form-control', 'required', 'placeholder' => 'Total PIX', 'min' => 0, 'step' => 'any' ]); !!}
          </div>
        </div>
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('total_debit_card', 'Tarjeta de Débito:*') !!}
              {!! Form::number('total_debit_card', $register_details->total_debit_card ?? 0, ['class' => 'form-control', 'required', 'placeholder' => 'Total Débito', 'min' => 0, 'step' => 'any' ]); !!}
          </div>
        </div>
        <div class="col-sm-4">
          <div class="form-group">
            {!! Form::label('total_credit_card', 'Tarjeta de Crédito:*') !!}
              {!! Form::number('total_credit_card', $register_details->total_credit_card ?? 0, ['class' => 'form-control', 'required', 'placeholder' => 'Total Crédito', 'min' => 0, 'step' => 'any' ]); !!}
          </div>
        </div>
        <hr>
    ...
    <div class="modal-footer no-print">
      <button type="button" class="tw-dw-btn tw-dw-btn-warning tw-text-white" onclick="window.print();" style="background-color: #f39c12; border-color: #e08e0b;">
