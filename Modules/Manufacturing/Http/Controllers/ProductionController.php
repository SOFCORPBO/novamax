    ...
    public function destroy($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                // 1. Buscar el production_sell asociado
                $production_sell = Transaction::where('business_id', $business_id)
                    ->where('type', 'production_sell')
                    ->where('mfg_parent_production_purchase_id', $transaction->id)
                    ->first();
                
                // 2. Si existe production_sell y está finalizado, devolver el stock
                if ($production_sell) {
                    if ($production_sell->status == 'final') {
                        // Obtener las sell_lines
                        $sell_lines = DB::table('transaction_sell_lines')
                            ->where('transaction_id', $production_sell->id)
                            ->get();
                        
                        foreach ($sell_lines as $sell_line) {
                            // Devolver la cantidad de ingredientes usados al stock
                            ...
                            // Actualizar mfg_quantity_used en las purchase_lines de ingredientes
                            $sell_line_purchase_lines = DB::table('transaction_sell_lines_purchase_lines')
                                ->where('sell_line_id', $sell_line->id)
                                ->get();
                            
                            foreach ($sell_line_purchase_lines as $slpl) {
                                DB::table('purchase_lines')
                                    ->where('id', $slpl->purchase_line_id)
                                    ->decrement('mfg_quantity_used', $slpl->quantity);
                            }
                            
                            // Eliminar las relaciones sell_line_purchase_lines
                    ...
                    // Eliminar sell_lines del production_sell
                    DB::table('transaction_sell_lines')
                        ->where('transaction_id', $production_sell->id)
                        ->delete();
                    
                    // Eliminar el production_sell
                    DB::table('transactions')
                        ->where('id', $production_sell->id)
                        ->delete();
                }
                
                // 3. Obtener las purchase_lines del producto final
                $purchase_lines = DB::table('purchase_lines')
                    ->where('transaction_id', $transaction->id)
                    ->get();
                
                // 4. Devolver el stock del producto final (restar porque se había sumado)
                ...
                // 5. Eliminar las purchase_lines del producto final
                DB::table('purchase_lines')
                    ->where('transaction_id', $transaction->id)
                    ->delete();
                
                // 6. Eliminar archivos/media asociados
                DB::table('media')
                    ->where('model_type', 'App\Transaction')
                    ->where('model_id', $transaction->id)
                    ->delete();
                
                // 7. Eliminar la transacción principal
                DB::table('transactions')
