...
$(document).ready(function () {
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            .autocomplete({
                ...
                response: function (event, ui) {
                    if (ui.content.length == 1) {
                        ...
                        // MANTENEMOS TU LÓGICA BASE INTACTA
                        ...
                        if (
                            ...
                            // 🔴 AQUÍ ESTÁ EL CAMBIO:
                            // Comentamos (desactivamos) estas dos líneas para que NO se agregue solo.
                            // Ahora el sistema respetará el stock, pero esperará a que tú le des clic.
            ...
            // ==========================================
            // 🟢 LÓGICA DE IMAGEN EN BUSCADOR
            // ==========================================
            var img_src = '/img/default_product.png';

            if (item.product_image && item.product_image !== 'null' && item.product_image !== '') {
                if (
                    item.product_image.indexOf('http') === 0 ||
                    item.product_image.indexOf('/') === 0
                ) {
                    img_src = item.product_image;
                } else {
                    img_src = '/uploads/img/' + item.product_image;
                }
            }

            var img_html =
                '<div style="float:left; margin-right:10px; width:45px; height:45px; display:flex; align-items:center; justify-content:center; overflow:hidden;">' +
                '<img src="' +
                img_src +
                '" onerror="this.src=\'/img/default_product.png\'" style="max-width:100%; max-height:100%; object-fit:contain; border-radius:4px; border:1px solid #ddd;">' +
                '</div>';
            // ==========================================

            var item_name = item.name;
            if (item.type == 'variable') {
                item_name += '-' + item.variation;
            }
            ...
            var price_formatted = __currency_trans_from_en(
                selling_price,
                false,
                false,
            ...
            if (
                ...
                var string =
                    '<li class="ui-state-disabled"><div style="padding:5px; border-bottom:1px solid #eee; overflow:hidden;">' +
                    img_html +
                    '<div style="margin-left: 55px; line-height: 1.2;"><strong>' +
                    item_name +
                    '</strong>' +
                    ' (' +
                    item.sub_sku +
                    ')<br> Precio: ' +
                    price_formatted +
                    ' <strong class="text-danger">(Agotado)</strong></div></div></li>';
                return $(string).appendTo(ul);
            } else {
                var string =
                    '<div style="padding:5px; border-bottom:1px solid #eee; overflow:hidden;">' +
                    img_html +
                    '<div style="margin-left: 55px; line-height: 1.2;"><strong>' +
                    item_name +
                    '</strong>' +
                    ' (' +
                    item.sub_sku +
                    ')<br> <span class="text-success" style="font-weight:bold;">Precio: ' +
                    price_formatted +
                    '</span>';
...
function calculate_billing_details(price_total) {
    // --- INICIO BLINDAJE: FORZAR LECTURA PURA ---
    // Esto evita que el sistema se auto-sume el recargo infinitamente
    var pure_subtotal = 0;
    $('table#pos_table tbody tr').each(function () {
        pure_subtotal += __read_number($(this).find('input.pos_line_total'));
    });
    price_total = pure_subtotal;
    // --- FIN BLINDAJE ---
    ...
    // =======================================================================
    // INICIO MODIFICACIÓN: CALCULAR RECARGO LIMPIO
    // =======================================================================
    try {
        var surcharge_percent = 0;
        var payment_method = $('.payment_types_dropdown').first().val();

        if (
            typeof window.__payment_surcharges !== 'undefined' &&
            window.__payment_surcharges[payment_method]
        ) {
            surcharge_percent = parseFloat(window.__payment_surcharges[payment_method]);
        }

        var exp_key = $('input[name="additional_expense_key_4"]');
        var exp_val = $('input[name="additional_expense_value_4"]');

        // Creamos los inputs nativos solo si no existen
        if (exp_key.length === 0) {
            ...
            exp_key = $('input[name="additional_expense_key_4"]');
            exp_val = $('input[name="additional_expense_value_4"]');
        }

        var surcharge_amount = 0;
        if (surcharge_percent > 0) {
            // Calcula sobre el subtotal puro
            var base_for_surcharge = price_total + order_tax - discount + shipping_charges;
            surcharge_amount = (base_for_surcharge * surcharge_percent) / 100;

            exp_key.first().val('Recargo Pago (' + surcharge_percent + '%)');
            __write_number(exp_val.first(), surcharge_amount);
        } else {
            exp_key.first().val('');
            __write_number(exp_val.first(), 0);
        }

        // Actualiza el texto rojo inferior
        if ($('#surcharge_pos_column').length > 0) {
            if (surcharge_amount > 0) {
                $('#surcharge_pos_column').show();
                $('#surcharge_pos_percent').text(surcharge_percent);
                $('#surcharge_pos_text').text(__currency_trans_from_en(surcharge_amount, true));
            } else {
                $('#surcharge_pos_column').hide();
            }
        }
    } catch (e) {
        console.error('Error recargo: ', e);
    }
    // ================= FIN MODIFICACIÓN ====================================

    // Aquí Ultimate POS suma todos los gastos, incluyendo nuestro recargo perfecto
    var additional_expense = 0;
    if ($('input[name="additional_expense_value_1"]').length > 0) {
        additional_expense += __read_number($('input[name="additional_expense_value_1"]').first());
    }
    if ($('input[name="additional_expense_value_2"]').length > 0) {
        additional_expense += __read_number($('input[name="additional_expense_value_2"]').first());
    }
    if ($('input[name="additional_expense_value_3"]').length > 0) {
        additional_expense += __read_number($('input[name="additional_expense_value_3"]').first());
    }
    if ($('input[name="additional_expense_value_4"]').length > 0) {
        additional_expense += __read_number($('input[name="additional_expense_value_4"]').first());
    }
